...
void generateGrid(float gridSize, int gridDivisions, std::vector<Vertex>& vertices, std::vector<uint32_t>& indices) {
    ...
    int gridLines = gridDivisions + 1;
    vertices.reserve(gridLines * gridLines);
    indices.reserve(gridLines * gridDivisions * 4);

    float yCoord = -gridSize / 2;
    for (int i = 0; i < gridLines; ++i) {
        float xCoord = -gridSize / 2;
        for (int j = 0; j < gridLines; ++j) {
            Vertex v{ {xCoord, yCoord, 0.0f}, {0.7f, 0.7f, 0.7f} };
    ...
    for (int i = 0; i < gridLines; ++i) {
        for (int j = 0; j < gridDivisions; ++j) {
            indices.push_back(gridLines * i + j);
            indices.push_back(gridLines * i + j + 1);
        }
    }

    for (int j = 0; j < gridLines; ++j) {
        for (int i = 0; i < gridDivisions; ++i) {
            indices.push_back(i * gridLines + j);
            indices.push_back((i + 1) * gridLines + j);
        }
    }
}

void generateSphere(float radius, int sectors, int stacks, std::vector<Vertex>& vertices, std::vector<uint32_t>& indices, glm::vec3 color = {1.0f, 1.0f, 1.0f}) {
    vertices.reserve((stacks + 1) * sectors);
    indices.reserve(stacks * sectors * 6);

    for (int i = 0; i <= stacks; ++i) {
        float theta = (static_cast<float>(i) / stacks) * glm::pi<float>();
        float sinTheta = std::sin(theta);
        float cosTheta = std::cos(theta);

        for (int j = 0; j < sectors; ++j) {
            float phi = (static_cast<float>(j) / sectors) * 2.0f * glm::pi<float>();
            float sinPhi = std::sin(phi);
            float cosPhi = std::cos(phi);

            Vertex v{ {radius * sinTheta * cosPhi, radius * sinTheta * sinPhi, radius * cosTheta}, {color} };
            vertices.push_back(v);
        }
    }
    ...
    for (int i = 0; i < stacks; ++i) {
        for (int j = 0; j < sectors; ++j) {
            uint32_t v0 = i * sectors + j;
            uint32_t v1 = i * sectors + (j + 1) % sectors;
            uint32_t v2 = (i + 1) * sectors + j;
            uint32_t v3 = (i + 1) * sectors + (j + 1) % sectors;
...
private:
    ...
    std::vector<VkBuffer> planetDataBuffers;
    std::vector<VkDeviceMemory> planetDataBuffersMemory;
    std::vector<void*> planetDataBuffersMapped;
    ...
    std::vector<uint32_t> planetIndices;
    ...
    void handleInput() {
        ...
        if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS) {
            camera.distance += cameraSpeed;
            if (camera.distance > 1000.0f) camera.distance = 1000.0f;
        }
    ...
    static void mouseCallback(GLFWwindow* window, double xpos, double ypos) {
        auto app = reinterpret_cast<SolarSystemApplication*>(glfwGetWindowUserPointer(window));
        if (app->mouseButtonHeld) {
            float dx = static_cast<float>(xpos - app->lastMouseX) * app->mouseSensitivity;
            float dy = static_cast<float>(ypos - app->lastMouseY) * app->mouseSensitivity;
            app->camera.yaw -= dx;
            app->camera.pitch += dy;
            if (app->camera.pitch > glm::radians(89.0f)) app->camera.pitch = glm::radians(89.0f);
            if (app->camera.pitch < glm::radians(-89.0f)) app->camera.pitch = glm::radians(-89.0f);
        }
        ...
        if (app->middleMouseButtonHeld) {
            ...
            glm::vec3 cameraPos;
            cameraPos.x = app->camera.distance * cos(app->camera.pitch) * cos(app->camera.yaw);
            cameraPos.y = app->camera.distance * cos(app->camera.pitch) * sin(app->camera.yaw);
            cameraPos.z = app->camera.distance * sin(app->camera.pitch); 
            cameraPos += app->camera.target;

            glm::vec3 forward = glm::normalize(app->camera.target - cameraPos);
            glm::vec3 right = glm::normalize(glm::cross(forward, glm::vec3(0.0f, 0.0f, 1.0f)));
            glm::vec3 up = glm::normalize(glm::cross(right, forward));

            glm::vec3 pan = (right * -dx + up * dy) * app->panSpeed;

            app->camera.target += pan;
        }
    ...
    static void mouseButtonCallback(GLFWwindow* window, int button, int action, int mods) {
        ...
        if (button == GLFW_MOUSE_BUTTON_LEFT) {
            app->mouseButtonHeld = (action == GLFW_PRESS);
        }
        if (button == GLFW_MOUSE_BUTTON_MIDDLE or button == GLFW_MOUSE_BUTTON_RIGHT) {
            app->middleMouseButtonHeld = (action == GLFW_PRESS);
        }
    }

    static void scrollCallback(GLFWwindow* window, double xoffset, double yoffset) {
        auto app = reinterpret_cast<SolarSystemApplication*>(glfwGetWindowUserPointer(window));
        app->camera.distance -= static_cast<float>(yoffset) * app->cameraSpeed;
        if (app->camera.distance < 0.5f) app->camera.distance = 0.5f;
        if (app->camera.distance > 1000.0f) app->camera.distance = 1000.0f;
    }
    ...
    void initVulkan() {
        ...
        createIndexBuffer(planetIndices, planetIndexBuffer, planetIndexBufferMemory);

        generateGrid(200.0f, 100, gridVertices, gridIndices);
        createVertexBuffer(gridVertices, gridVertexBuffer, gridVertexBufferMemory);
        createIndexBuffer(gridIndices, gridIndexBuffer, gridIndexBufferMemory);

        createPlanetDataBuffers();
    ...
    void cleanup() {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            vkDestroyBuffer(device, planetDataBuffers[i], nullptr);
            vkFreeMemory(device, planetDataBuffersMemory[i], nullptr);
        }
    ...
    void createComputePipeline() {
        auto computeShaderCode = readFile("shaders/physics.comp.spv");
    ...
    void createPlanetDataBuffers() {
        VkDeviceSize bufferSize = sizeof(PlanetData) * planets.size();
        planetDataBuffers.resize(MAX_FRAMES_IN_FLIGHT);
        planetDataBuffersMemory.resize(MAX_FRAMES_IN_FLIGHT);
        planetDataBuffersMapped.resize(MAX_FRAMES_IN_FLIGHT);

        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            createBuffer(bufferSize, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, planetDataBuffers[i], planetDataBuffersMemory[i]);
            vkMapMemory(device, planetDataBuffersMemory[i], 0, bufferSize, 0, &planetDataBuffersMapped[i]);

            PlanetData* planetData = static_cast<PlanetData*>(planetDataBuffersMapped[i]);
            for (size_t j = 0; j < planets.size(); j++) {
                planetData[j].position = planets[j].position;
                planetData[j].velocity = planets[j].velocity;
                planetData[j].color = planets[j].color;
                planetData[j].diameter = planets[j].diameter;
                planetData[j].mass = planets[j].mass;
            }
        }
    }
    ...
    void createDescriptorSets() {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            VkDescriptorBufferInfo planetDataInfo{};
            planetDataInfo.buffer = planetDataBuffers[i];
            planetDataInfo.offset = 0;
            planetDataInfo.range = sizeof(PlanetData) * planets.size();
            ...
            descriptorWrites[0].pBufferInfo = &planetDataInfo;
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
        ...
        uint32_t workGroupCount = (planets.size() + 63) / 64;
        ...
        vkCmdBindVertexBuffers(commandBuffer, 0, 1, &planetVertexBuffer, &offset);
        vkCmdBindIndexBuffer(commandBuffer, planetIndexBuffer, 0, VK_INDEX_TYPE_UINT32);
        vkCmdDrawIndexed(commandBuffer, static_cast<uint32_t>(planetIndices.size()), planets.size(), 0, 0, 0);
    ...
    void updateFPS() {{
            ...
            if (timeAccumulated >= 1.0f) {
                fps = frameCount / timeAccumulated;
                frameCount = 0;
